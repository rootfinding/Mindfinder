...
class PineconeController:
    def query(text, filter, namespace, top_k=10):
        ...
        query_vector = embed.embed_documents([text])[0]
        results = index.query(vector=query_vector, top_k=top_k, namespace=namespace, filter=filter,
    ...
    def upload(text, metadata, namespace):
        ...
        index.upsert([(id, embeds[0], metadata)], namespace=namespace)
